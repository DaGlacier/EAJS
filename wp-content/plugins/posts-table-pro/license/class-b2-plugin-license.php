...
if ( ! class_exists( 'Barn2_Plugin_License' ) ) {
    ...
    class Barn2_Plugin_License {
                ...
                if ( $this->is_api_error( $response ) ) {
                    ...
                    if ( false === $license_data->success ) {
                        switch ( $license_data->error ) {
                            case 'missing' :
                                $error_message = __( 'Please enter your license key.', 'posts-table-pro' );
                                break;
                            case 'missing_url' :
                                $error_message = __( 'No URL was supplied for activation.', 'posts-table-pro' );
                                break;
                            case 'license_not_activable' :
                                $error_message = __( 'This license is for a bundled product and cannot be activated.', 'posts-table-pro' );
                                break;
                            case 'expired' :
                            ...
                            case 'invalid_item_id':
                                $error_message = __( 'Your license key is not valid for this plugin.', 'posts-table-pro' );
                                break;
                            case 'no_activations_left':
                                $error_message = __( 'Your license key has reached its activation limit.', 'posts-table-pro' );
                                break;
                            case 'disabled':
                                $error_message = __( 'Your license key has been disabled, please contact support,', 'posts-table-pro' );
                                break;
                            case 'key_mismatch':
                                $error_message = __( 'License key mismatch, please contact support.', 'posts-table-pro' );
                                break;
                            default :
        ...
        public function get_license_key_admin_message() {
            ...
            $deactivate_button = sprintf( '<button type="submit" class="button b2-license-button" name="deactivate_key" value="%1$s" style="margin-left:8px;">%2$s</button>', esc_attr( $this->license_key_option ), __( 'Deactivate', 'posts-table-pro' ) );
