...
class Posts_Data_Table {
    ...
    public function update( array $new_args ) {
        ...
        if ( array_intersect( $args_changed, array( 'post_type', 'status', 'year', 'month', 'day', 'category', 'tag', 'term', 'cf', 'author', 'exclude',
                'include', 'exclude_category', 'search_term' ) ) ) {
        ...
        if ( array_intersect( $args_changed, array( 'search_filters' ) ) ) {
            // If the search  filters changed from last time, reset posts array and filtered total, but leave the overall total.
            $this->query->set_posts( null );
            $this->query->set_total_filtered_posts( null );
        }
        ...
        if ( $this->args->lazy_load && array_intersect( $args_changed, array( 'status', 'year', 'month', 'day', 'category', 'tag', 'term', 'cf', 'author',
                'exclude', 'include', 'exclude_category', 'rows_per_page', 'post_limit', 'sort_by', 'sort_order', 'search_term', 'search_filters' ) ) ) {
